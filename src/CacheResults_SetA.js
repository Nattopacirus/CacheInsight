...
const calculateSetAssociativeCache = (cacheSize, blockSize, fileData, associativity, replacementPolicy, addressSize) => {
    const cacheSizeBytes = Math.floor(cacheSize) * 1024; // ใช้ Math.floor เพื่อป้องกันทศนิยม
    const blockSizeBytes = Math.floor(blockSize); // ใช้ Math.floor เพื่อป้องกันทศนิยม
    const associativityValue = Math.floor(associativity); // ใช้ Math.floor เพื่อป้องกันทศนิยม
    ...
    if (associativityValue <= 0 || !Number.isInteger(associativityValue)) throw new Error("Associativity must be a positive integer.");

    const numberOfSets = Math.floor(cacheSizeBytes / blockSizeBytes / associativityValue); // ใช้ Math.floor เพื่อให้แน่ใจว่าเป็นจำนวนเต็ม
    if (numberOfSets < 1) {
        throw new Error(`Invalid cache configuration: number of sets (${numberOfSets}) must be a positive integer. Check cache size (${cacheSize} KB), block size (${blockSize} B), and associativity (${associativity}).`);
    }
    ...
    fileData.forEach((row, rowIndex) => {
        ...
        if (setIndex >= 0 && setIndex < numberOfSets && cache[setIndex]) {
            if (cache[setIndex].includes(tag)) {
                ...
                misses++;
                if (cache[setIndex].length < associativityValue) {
            ...
            invalidRows++;
            console.warn(`Row ${rowIndex}: Invalid setIndex ${setIndex} for address ${addressHex}`);
        }
    });
