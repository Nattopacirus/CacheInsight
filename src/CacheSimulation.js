...
const CacheSimulation = () => {
    ...
    const [isLoading, setIsLoading] = useState(false); // สำหรับการโหลด simulation
    const [isUploading, setIsUploading] = useState(false); // สำหรับการอัปโหลดไฟล์
    ...
    const handleFileUpload = (event) => {
        ...
        setIsUploading(true); // แสดง modal "กำลังอัปโหลดไฟล์"
        ...
        reader.onload = (e) => {
            ...
            const result = Papa.parse(text, {
                ...
                complete: (parsedResult) => {
                    setIsUploading(false); // ปิด modal เมื่ออัปโหลดเสร็จ
    ...
    const startSimulation = async () => {
        ...
        for (let i = 0; i < totalRows; i++) {
            await new Promise((resolve) => setTimeout(resolve, 0)); // ให้เบราว์เซอร์มีเวลารีเฟรช UI
        }
    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 to-blue-100 p-6">
            {/* Modal สำหรับแสดงข้อความ "กำลังอัปโหลดไฟล์" */}
            {isUploading && (
                <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
                    <div className="bg-white p-8 rounded-2xl shadow-xl text-center">
                        <h1 className="text-2xl font-semibold text-blue-700 mb-4">Uploading file...</h1>
                        <p className="text-md text-gray-500">please wait a moment</p>
                    </div>
                </div>
            )}
